export const topics = [
  ...
  },
  {
    ...
    keyPoints: [
      "Relation: en mängd tupler där varje element tillhör en domän — fakta av ett slag. Attribut: ett namn parat med en domän. Tupel: en komplett fakta med ett värde per attribut, där inga två element har samma attributnamn. Relation är inte samma sak som ER-relationship: EMPLOYEE lagrar entitetsfakta, WORKS_ON relationsfakta, båda är relationer (slide 9).",
      "Schema och aktuellt värde (slide 12–13): schemat EMPLOYEE(EmployeeNo, Name, WorkEmail) bestämmer formen på varje giltig tupel; det aktuella värdet är tuplerna just nu och kan växa, krympa eller vara tomt. En ny anställd är en ny tupel, aldrig ett nytt attribut.",
      "Domän (slide 14): mängden tillåtna värden enligt schemat, inte de värden som redan används — E-555 är tillåtet fast ingen tupel har det. Inte samma sak som datatyp: domänen bär affärsregeln (lön 10 000–30 000), datatypen säger bara INT.",
      "Grad och kardinalitet (slide 15–16): grad = antalet attribut, ändras bara med schemat; kardinalitet = antalet tupler just nu, tomt värde = 0. ER-modellens kardinalitet är ett annat begrepp.",
      "Terminologi (slide 11): tupel/rad/post, attribut/kolumn/fält; äldre material även relation/tabell/fil. 'Fält' kan betyda ett enskilt värde eller en kolumn beroende på källa.",
      "Relationens sju egenskaper: unikt namn; atomära värden i varje cell; distinkta attributnamn (slide 7); samma datatyp och domän inom ett attribut; attributens ordning saknar betydelse (slide 18); tuplernas ordning saknar betydelse (slide 17); inga dubblettupler (slide 19) — enskilda värden får upprepas, den kompletta tupeln får det inte.",
      "SQL ger en bag, relationen är en mängd (slide 20–21): SELECT Name kan ge två identiska Mary-rader; bagen räknar upprepningar, relationen innehåller tupeln en gång, DISTINCT tar bort dubblettraderna. Varken mängd eller bag har ordning — utan ORDER BY är radordningen inte garanterad. Kravet på atomära värden är samma krav som 1NF."
    ],
    pitfalls: [
  ...
  },
  {
    ...
    keyPoints: [
      "Unikhet är en verksamhetsregel (slide 23–24): den gäller varje tillåten population. Aktuella rader kan motbevisa en identifierare men aldrig bevisa en regel — leta efter regeln i uppgiftstexten, inte efter mönster i exempeldatan.",
      "Kandidatnyckel (candidate key, slide 26): en attributmängd K som uppfyller både unikhet (inga två skilda tupler har samma värden i K i något giltigt relationsvärde) och minimalitet (inget attribut kan tas bort ur K utan att unikheten förloras). {EmployeeNo, Name} är unik men inte minimal. En relation kan ha flera, en per unikhetsregel; en sammansatt kandidatnyckel som {EmployeeNo, ProjectNo} är unik som par, inte var för sig, och alla attributen stryks under.",
      "Primärnyckel (primary key, slide 29–30): en utvald kandidatnyckel, PK = CK1. Valet skapar ingen ny kandidatnyckel och tar inte bort någon — WorkEmail förblir CK2 och måste vara unik. Välj stabil, minimal och semantiskt meningsfull.",
      ...
      "Notation (slide 34): CK1 = {…}, CK2 = {…}, PK = CK1, FK1 : (LeaderEmployeeNo) REF EMPLOYEE(EmployeeNo) — före REF den refererande attributlistan, efter REF den refererade relationen och nyckeln. Häftets facit stryker under: hel linje för PK, prickad för FK; tentans uppgift 3 kräver understrykningen.",
  ...
  },
  {
    ...
    keyPoints: [
      "Vad som produceras (slide 39, 41): relationsscheman, nycklar och constraints — inte SQL och inte fyllda tabeller. Sex regler i ordning (slide 42): vanliga entiteter, svaga entiteter, 1:1, 1:N, M:N, flervärdesattribut. Entitetsrelationer och nycklar först, resten som checklista; alla modeller behöver inte alla sex.",
      "Regel 1, vanlig entitet (slide 46–47): en relation med alla enkla attribut och de enkla delarna av sammansatta attribut; varje identifierare blir kandidatnyckel, en väljs som primärnyckel. Flera identifierare ger CK1 och CK2 — att välja en tar inte bort unikhetskravet på den andra (slide 50). Sammansatt identifierare: alla delar tillsammans, inget separat ProjectNo (slide 51–52).",
      "Regel 2, svag entitet (slide 56–57): ägaren mappas först; relationen får de enkla attributen, ägarens primärnyckel som främmande nyckel, den identifierande relationens attribut (addedAt, slide 62), och primärnyckel = ägarens nyckel + partiell nyckel. Ingen egen relation för den identifierande relationen (slide 59). Sammansatt ägarnyckel tas med hel, som en främmande nyckel (slide 63).",
      "Kedjade svaga entiteter (slide 64–65): varje led refererar sin närmaste ägares kompletta nyckel — TASK_STEP(ProjectNo, TaskNo, StepNo) med FK (ProjectNo, TaskNo) REF PROJECT_TASK — inte TaskNo ensamt.",
      "Regel 3, binär 1:1 (slide 69–70): främmande nyckel i den ena relationen, helst hos den sida som deltar totalt; den främmande nyckeln görs till KANDIDATNYCKEL, annars bevaras inte 1:1 (slide 74); kräv referens där deltagandet är totalt; relationsattributen i värdrelationen (slide 76). Båda totala: valfri sida med totalt deltagande upprätthållet på båda håll, eller sammanslagning till en relation där båda identifierarna förblir kandidatnycklar (slide 77–78). Ett tredje, sällan föredraget alternativ: en separat relation.",
      "Regel 4, binär 1:N (slide 84–85): 1-sidans primärnyckel som främmande nyckel i N-sidans relation; värdet får upprepas — den främmande nyckeln är INGEN kandidatnyckel, det är skillnaden mot 1:1 (slide 88, 93); relationsattributen i N-sidans relation (slide 91). Samma regel på en redan mappad svag relation utan att dess nyckel ändras.",
      "Regel 5, binär M:N (slide 96–97): ny relation med båda deltagarnas primärnycklar som främmande nycklar, kombinationen som primärnyckel, relationsattributen utanför nyckeln — samma par två gånger med olika värden är en nyckelöverträdelse (slide 107). Deltagandekraven hanteras separat: alla nycklar kan hålla medan ett projekt saknar tupel (slide 103).",
      "Sammansatt främmande nyckel (slide 108–110): en deltagare med sammansatt nyckel kopieras hel som EN referens, FK2 : (RegistrationYear, SequenceNo) REF PROJECT(…) — inte två oberoende, för SequenceNo ensamt är ingen projektnyckel.",
      "Regel 6, flervärdesattribut (slide 114–115): ny relation med ägarens hela primärnyckel som främmande nyckel plus attributet; kombinationen är primärnyckel. Hela samlingen i en cell är en domänöverträdelse (slide 117). En ägare utan värden har ingen tupel; ett nytt värde är en ny tupel, aldrig en ny kolumn (slide 119–122). Konsekvens: värdet kan delas mellan flera ägare.",
