...
// som ändar. Samma tokens som Chen-figurerna (pine, brass, ink).
const PINE = "var(--pine)";
...
function KeyIcon({ x, y }) {
  return (
    <g stroke={BRASS} strokeWidth={1.4} fill="none">
      <circle cx={x + 4} cy={y - 3} r={3} />
...
export function VpEntity({ x, y, w = 156, label, attrs }) {
  ...
  return (
    <g>
      <rect x={x} y={y} width={w} height={h} rx={10} fill="white" stroke={PINE} strokeWidth={1.6} />
      <rect x={x + 1} y={y + 1} width={w - 2} height={HEAD} rx={9} fill="rgba(31,78,69,0.08)" />
      <line x1={x} x2={x + w} y1={y + HEAD} y2={y + HEAD} stroke={PINE} strokeWidth={1.2} />
      <text x={x + w / 2} y={y + HEAD / 2 + 1} textAnchor="middle" dominantBaseline="central" fontWeight="600" fill={INK}>
        {label}
      </text>
      {attrs.map((attr, i) => {
        ...
        return (
          <g key={attr.name} fontSize={12.5} fill={INK}>
            ...
            {attr.nullable && (
              <g>
                <rect x={x + w - 22} y={cy - 7} width={14} height={14} rx={2} fill="white" stroke={SOFT} strokeWidth={1} />
                <text x={x + w - 15} y={cy + 0.5} textAnchor="middle" dominantBaseline="central" fontSize={10} fill={SOFT} fontWeight="700">N</text>
              </g>
            )}
          </g>
        );
...
export function VpLine({ x1, x2, y, label, left, right }) {
  return (
    <g>
      <line x1={x1} y1={y} x2={x2} y2={y} stroke={PINE} strokeWidth={1.4} strokeDasharray="6 4" />
      <VpEnd x={x1} y={y} dir={1} {...left} />
      <VpEnd x={x2} y={y} dir={-1} {...right} />
      {label && (
        <text x={(x1 + x2) / 2} y={y - 8} textAnchor="middle" fontSize={12.5} fontWeight="600" fill={INK}>
          {label}
        </text>
      )}
    </g>
